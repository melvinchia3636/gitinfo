    ...
    fetch('https://api.github.com/rate_limit', FETCH_PARAMS).then((res) => res.json()).then(({ resources: { core } }) => {
      if (core.remaining) {
        fetch(`https://api.github.com/repos/${params.user}/${params.reponame}`, FETCH_PARAMS).then((res) => res.json()).then(async (d) => {
          ...
          const README_URL = ['README', 'readme', '.github/README'].map((e) => `https://cors-anywhere.thecodeblog.net/raw.githubusercontent.com/${d.full_name}/${d.default_branch}/${e}.md`);
          let readmeContent;
          ...
          setData({
            ...d, langs, contributors, subscribers, stargazers, contributorsCount, readmeContent,
          });
  ...
  return (
    <div className="h-full w-full pt-4 pb-0 overflow-scroll">
      {JSON.stringify(data) !== '{}' ? (
        data !== 'finished' ? (
          <div className="pb-8">
            <div className="flex gap-2 items-center text-slate-600 dark:text-gray-100 text-3xl tracking-wide font-medium">
            ...
            {data.description || data.homepage ? (
              <div className="mt-8">
                <div className="flex items-center gap-2 text-2xl font-medium text-slate-600 dark:text-gray-100 tracking-wide">
                  <Icon icon="uil:info-circle" className="w-8 h-8 text-indigo-500 dark:text-indigo-400" />
                  Description
                </div>
                <p className="mt-3 text-slate-600 dark:text-gray-100 text-lg ml-1 tracking-wide">{data.description}</p>
                {data.homepage ? (
                  <a href={data.homepage} target="_blank" rel="noreferrer" className="text-indigo-500 dark:text-indigo-400 mt-2 font-medium flex items-center gap-2 ml-1 tracking-wide">
            ...
            <div className="mt-8">
              ...
              <p className="mt-3 text-slate-600 dark:text-gray-100 text-lg ml-1 tracking-wide">{new Date(data.created_at).toLocaleString()}</p>
            </div>
            <div className="mt-8">
              <div className="flex items-center gap-2 text-2xl font-medium text-slate-600 dark:text-gray-100 tracking-wide">
            ...
            <div className="mt-8">
              ...
              <div className="mt-3 flex w-full bg-slate-500 h-4 l rounded-full overflow-hidden">
                {Object.entries(data.langs).map(([k, v]) => (
                  <div className="h-full bg-indigo-500" style={{ backgroundColor: colors[k]?.color }} data-key={k} data-value={v} />
                ))}
              </div>
            ...
            <div className="mt-8">
              ...
              {nextContributorsPage ? (
                <button onClick={fetchNextContributorsPage} type="button" className="text-lg text-white h-14 w-full bg-indigo-500 rounded-md shadow-md mt-6">
            ...
            <div className="mt-8">
              ...
              {nextSubscribersPage ? (
                <button onClick={fetchNextSubscribersPage} type="button" className="text-lg text-white h-14 w-full bg-indigo-500 rounded-md shadow-md mt-6">
            ...
            {data.stargazers.length ? (
              <div className="mt-8">
                ...
                {nextStargazersPage ? (
                  <button onClick={fetchNextStargazersPage} type="button" className="text-lg text-white h-14 w-full bg-indigo-500 rounded-md shadow-md mt-6">
                    {isStargazersLoading ? (
                      <Lottie
                        options={{
                          loop: true,
                ...
                <div className="markdown-body mt-4 text-slate-600 dark:text-gray-100 !break-words">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      h5: HeadingRenderer,
                      h6: HeadingRenderer,
                      a: (props) => <a href={props.href} target={!props.href.startsWith('#') ? '_blank' : ''} rel="noreferrer">{props.children}</a>,
                      code({
