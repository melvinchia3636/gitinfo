    ...
    <div className="h-full w-full flex items-center justify-center pb-8">
      <div className="bg-white dark:bg-zinc-600 rounded-lg shadow-lg overflow-hidden w-1/2">
        <form onSubmit={fetchResult} className="flex items-center gap-4">
          ...
          <input onChange={(e) => setQuery(e.target.value)} type="text" placeholder="Search Github" className="placeholder-zinc-300 caret-indigo-500 py-4 w-full text-xl focus:border-none focus:outline-none text-zinc-600 dark:text-zinc-200 bg-white dark:bg-zinc-600" />
                  ...
                  <div>
                    ...
                    <div>
                      {result.users.items.slice(0, 5).map((e) => (
                        <Link to={`/user/${e.login}`} className="flex items-center gap-4 py-2 border-b border-zinc-50 dark:border-zinc-600 px-2 hover:bg-purple-50 dark:hover:bg-zinc-500 transition-all duration-200 hover:rounded-md">
                  ...
                  <div>
                    ...
                    <div>
                      {result.repo.items.slice(0, 5).map((e) => (
                        <Link to={`/repo/${e.full_name}`} className="flex items-center gap-3 py-2 border-b border-zinc-50 dark:border-zinc-600 px-2 hover:bg-purple-50 dark:hover:bg-zinc-500 transition-all duration-200 hover:rounded-md">
